...
NOTE:
- These utilities are primarily used in documentation and
    alternative/backup experiment scripts (e.g., experiments copy/).
- The core rolling_agent_forecast.py pipeline does not import
    this module directly, but it remains the canonical place for
...
def update_conformal_buffer(buffer: List[float], residual: Optional[float], max_size: int = 52) -> List[float]:
    if residual is None or not np.isfinite(residual):
        return buffer
    new_buffer = list(buffer)
    new_buffer.append(float(residual))
    if len(new_buffer) > max_size:
        new_buffer = new_buffer[-max_size:]
    return new_buffer


def conformal_radius(residuals: List[float], alpha: float = 0.05) -> Optional[float]:
    if not residuals:
        return None
    arr = np.asarray([r for r in residuals if np.isfinite(r)], dtype=float)
    if arr.size == 0:
        return None
    idx = int(np.ceil((arr.size + 1) * (1.0 - alpha)) - 1)
    idx = max(0, min(idx, arr.size - 1))
    arr.sort()
    return float(arr[idx])


def conformalize_quantiles(q05: np.ndarray, q50: np.ndarray, q95: np.ndarray, radius: Optional[float]) -> Dict[str, np.ndarray]:
    q05_arr = np.asarray(q05, dtype=float)
    q50_arr = np.asarray(q50, dtype=float)
    q95_arr = np.asarray(q95, dtype=float)
    if radius is None or radius <= 0:
        return {"q05": q05_arr, "q95": q95_arr}
    lower = np.minimum(q05_arr, q50_arr - radius)
    upper = np.maximum(q95_arr, q50_arr + radius)
    lower = np.maximum(lower, 0.0)
    upper = np.maximum(upper, lower)
    return {"q05": lower, "q95": upper}


def ensemble_disagreement(paths: np.ndarray) -> Dict[str, Any]:
    x = np.asarray(paths, dtype=float)
    if x.ndim != 2 or x.size == 0:
        return {"mean_pct": 0.0, "max_pct": 0.0, "series": []}
    median = np.median(x, axis=0)
    mad = np.median(np.abs(x - median), axis=0)
    denom = np.maximum(1.0, np.abs(median) + 1.0)
    rel = np.clip(mad / denom, 0.0, None)
    mean_pct = float(np.mean(rel) * 100.0)
    max_pct = float(np.max(rel) * 100.0) if rel.size else 0.0
    return {"mean_pct": mean_pct, "max_pct": max_pct, "series": rel.tolist()}
